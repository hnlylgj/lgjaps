...
namespace SmartBusECOService.FTP
{
    public class FTPSocketHelper
    {

        //-----------------------------------------
        public TcpListener MyTcpListener = null;
        private Thread listenThread;
        private string LocalAddress = "168.223.110.110";
        private int LocalPort=21;
        //保存用户名和密码
        private Dictionary<string, string> AllUsers;
        private string UserPath;

        public delegate void OutMessageDelegate(string str);
        public OutMessageDelegate OutMessageCallBack; 
        //用于保存UI线程的同步上下文
        public static SynchronizationContext UIContext = null;
        public static SendOrPostCallback UpdateUIFuncEx = null;

        public FTPSocketHelper()
        {
            UserPath = "D:/MyFtpServerRoot/";

        }

        public void FTPInit(string IPStr, int Port, string InUserPath)
        {
        ...
        {
            MyTcpListener = new TcpListener(IPAddress.Parse(LocalAddress), LocalPort);
            // 开始监听传入的请求
            MyTcpListener.Start();   
            OutMessageCallBack("启动FTP服务成功！");  
            OutMessageCallBack("Ftp服务器运行中...[点击”停止“按钮停止FTP服务]");
            ...
            while (true)
            {
                try
                {
                    ...
                    OutMessageCallBack(string.Format("客户端({0})与本机({1})建立Ftp连接", MytcpClient.Client.RemoteEndPoint, MyTcpListener.LocalEndpoint));

                    User user = new User();
                    user.commandSession = new UserSeesion(MytcpClient);
                    user.workDir = UserPath;
                    Thread NewUserThread = new Thread(UserProcessing);
                    NewUserThread.IsBackground = true;
                    NewUserThread.Start(user);
                }
                catch
                {
                    break;
        ...
        private void CommandPassword(User user, string command, string param)
        {
            ...
            if (command == "PASS")
            {
                ...
                if (AllUsers.TryGetValue(user.userName, out password))
                {
                    if (password == param)
                    {
            ...
            while (true)
            {
                ...
                catch (Exception ex)
                {
                    if (user.commandSession.tcpClient.Connected == false)
                    {
                        ...
                        OutMessageCallBack(string.Format("客户端({0}断开连接！)", user.commandSession.tcpClient.Client.RemoteEndPoint));

                    }
                    else
                    {
                  
                        OutMessageCallBack("接收命令失败！" + ex.Message);
                    }
                ...
                if (receiveString == null)
                {
                    ...
                    OutMessageCallBack("接收字符串为null,结束线程！");

                    break;
                }

           
                OutMessageCallBack(string.Format("来自{0}：[{1}]", user.commandSession.tcpClient.Client.RemoteEndPoint, receiveString));
                // 分解客户端发来的控制信息中的命令和参数
        ...
        private void RepleyCommandToUser(User user, string str)
        {
            try
            {
                ...
                OutMessageCallBack(string.Format("向客户端({0})发送[{1}]", user.commandSession.tcpClient.Client.RemoteEndPoint, str));

            }
            catch
            {
               
                OutMessageCallBack(string.Format("向客户端({0})发送信息失败", user.commandSession.tcpClient.Client.RemoteEndPoint));
            }
        }
        ...
        // 处理LIST/NLIST命令，向客户端发送当前或指定目录下的所有文件名和子目录名
        ...
        private void CommandDELE(User user, string filename)
        {
            ...
            OutMessageCallBack("正在删除文件" + filename + "...");

            File.Delete(path);
            //AddInfo("删除成功");
            OutMessageCallBack("删除成功");
        ...
        {
            string sendString = string.Empty;
            IPAddress localip = IPAddress.Parse(LocalAddress);// Dns.GetHostEntry("").AddressList[1];--127.0.0.1
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    OutMessageCallBack("TCP 数据连接已打开（被动模式）--" + localip.ToString() + "：" + port);
                }
                catch
                {
                    continue;
        ...
        private void InitDataSession(User user)
        {
            ...
            if (user.isPassive)
            {
                ...
                OutMessageCallBack("采用被动模式返回LIST目录和文件列表");
                client = user.dataListener.AcceptTcpClient();
            }
            else
            {
              
                OutMessageCallBack("采用主动模式向用户发送LIST目录和文件列表");
        ...
        private void SendByUserSession(User user, string sendString)
        {
            ...
            OutMessageCallBack("向用户发送(字符串信息)：[" + sendString + "]");
            try
            {
                user.dataSession.streamWriter.WriteLine(sendString);
              
                OutMessageCallBack("发送完毕");
            }
            finally
            {
        ...
        private void ReadFileByUserSession(User user, FileStream fs)
        {
            ...
            OutMessageCallBack("接收用户上传数据（文件流）：[...");
            try
            {
                if (user.isBinary)
                {
        ...
        public void OutLogingPut(string SendReceiveString)
        {
